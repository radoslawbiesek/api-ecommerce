...
import { mockProduct, deleteMockProducts, generateMockProduct } from "tests/mock.js";
...
describe("Products Repository", () => {
  ...
  describe("create", () => {
    test("should create a product", async () => {
      ...
      const foundProduct = (await db.select().from(products).where(eq(products.id, createdProduct.id)))[0];
  ...
  describe("findById", () => {
    test("should return a product by id", async () => {
      const savedProduct = await mockProduct();
  ...
  describe("findAll", () => {
    test("should return all products", async () => {
      const savedProducts = [await mockProduct(), await mockProduct(), await mockProduct()];
  ...
  describe("update", () => {
    test("should update a product", async () => {
      const product = await mockProduct();

      product.name = "Updated Product";
      await productsRepository.update(product.id, product);
      const updatedProduct = (await db.select().from(products).where(eq(products.id, product.id)))[0];
  ...
  describe("delete", () => {
    test("should delete a product", async () => {
      ...
      const foundProduct = (await db.select().from(products).where(eq(products.id, product.id)))[0];
